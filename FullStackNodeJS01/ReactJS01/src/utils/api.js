...
export const addRecentlyViewedApi = (productId) => {
  const URL_API = `/v1/api/recently-viewed/${productId}`
  console.log('Adding to recently viewed:', productId)
  return axios.post(URL_API)
}

export const getRecentlyViewedApi = () => {
  const URL_API = `/v1/api/recently-viewed`
...
export const getProductStatsApi = (id) => {
  const URL_API = `/v1/api/products/${id}/stats`
  return axios.get(URL_API)
}
